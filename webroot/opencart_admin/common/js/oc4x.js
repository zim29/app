...
function open_manual_notification(message, class_custom, icon_class) {
	...
	container_append.prepend('<div class="alert alert-'+class_custom+'"><i class="fas fa-'+icon_class+'-circle"></i> '+message+'<button type="button" class="btn-close close" data-bs-dismiss="alert"></button></div>');
...
var options_autocomplete = {
	...
	select: function(item) {
		...
		var table_products = $(this).parent().find('table.table tbody');
		$(table_products).find('tr#element-' + item['value']).remove();

		var new_element = $('<tr id="element-' + item['value'] + '"></tr>');

		new_element.append('<td>' + item['label'] + '<input type="hidden" name="'+input_name+'[]" value="' + item['value'] + '" /></td>');
		new_element.append('<td class="text-end"><button type="button" class="btn btn-danger btn-sm"><i class="fas fa-minus-circle"></i></button></td>');

		$(table_products).append(new_element);
	}
};

$(document).on('ready', function() {
	load_products_autocomplete_inputs();
});

function load_products_autocomplete_inputs() { 
	$('input.products_autocomplete').autocomplete(options_autocomplete);

	$(document).on('click', 'div.products-autocomplete-table-container button.btn', function() {
