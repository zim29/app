...
$(function(){
	$('div.toogle_main_field').find('input[type="checkbox"]').click(function(){
		check_toogle_main_fields($(this));
	});
	check_toogle_main_fields_ready();
});

function check_toogle_main_fields_ready() {
	$('div.toogle_main_field').find('input[type="checkbox"]').each(function(){
		check_toogle_main_fields($(this));
	});
}

function check_toogle_main_fields(checkbox) {
	var checkbox_name = checkbox.attr('name');
	var checked = checkbox.is(':checked') && checkbox.closest('div.toogle_main_field').css('display') == 'block';
	var container = checkbox.closest('div.toogle_main_field');
    var store = container.data('store');

    if(typeof store !== 'undefined') {
		checkbox_name = checkbox_name.replace("_"+store, "");
		checkbox_name += '.store_'+store;
	}
    var selector = $('.toogle_seconday_field.'+checkbox_name);
	if(checked)
		selector.show();
	else
		selector.hide();
...
function ajax_loading_open(container) {
	if(typeof container !== 'undefined')
  		jQuery(container).prepend('<div class="ajax_loading container"><i class="fa fa-refresh fa-spin"></i></div>');
	else
		jQuery('body').prepend('<div class="ajax_loading"><i class="fa fa-refresh fa-spin"></i></div>');
}

function ajax_loading_close(container) {
	if(typeof container !== 'undefined')
  		jQuery(container).find('div.ajax_loading').fadeOut('fast', function(){
			jQuery(this).remove();
		});
	else
...
{
	var send_configuration = true;
	if(typeof save_configuration_ajax_extended === 'function')
		send_configuration = save_configuration_ajax_extended();
...
{
	var tab_help = $('div#tab-support').length ? $('div#tab-support input,div#tab-support textarea,div#tab-support select') : $('div#tab-Поддержка input,div#tab-Поддержка textarea,div#tab-Поддержка select');
...
function send_ticket_cs_cart()
{
	var tab_help = $('div#tab-support').length ? $('div#tab-support input,div#tab-support textarea,div#tab-support select') : $('div#tab-Поддержка input,div#tab-Поддержка textarea,div#tab-Поддержка select');

    var security_hash = $('#google_all input[name=security_hash]').val();
    var data = {security_hash:security_hash};
    tab_help.each(function(key, item){
