...
abstract class AbstractWorkbook implements WorkbookInterface
{
    ...
    public function addRowToCurrentWorksheet($dataRow, $style)
    {
        ...
        //NAQK:Hack
        //Register cell style before row style
        $styleHelper = $this->registerCellStyle($dataRow,$styleHelper);
        ...
        if ($hasReachedMaxRows) {
            ...
            if ($this->shouldCreateNewSheetsAutomatically) {
                ...
                $this->registerCellStyle($dataRow);
    ...
    /**
    * NAQK:Hack-20170914
    * go throw data to register cell style
    * do that before set row style
    * @param array $dataRow Array containing data to be written. Cannot be empty.
    *  Example $dataRow = [..., {'value':'acb','style': @style_not_registered}];
    * @param Box\Spout\Writer\Common\Helper $styleHelper StyleHelper to append
    * @return Box\Spout\Writer\Common\Helper $styleHelper StyleHelper to appended
    */
    protected function registerCellStyle($dataRow,$styleHelper)
    {
        foreach ($dataRow as $data) {
            if (is_array($data) && isset($data["value"]) && isset($data["style"]))
            {
                $data["style"] = $styleHelper->registerStyle($data["style"]);
            }
        };
        return $styleHelper;
    }
