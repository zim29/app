...
EOD;
    ...
    private function addNonEmptyRow($dataRow, $style)
    {
        ...
        foreach($dataRow as $cellValue) {
            ...
            //overwrite row's style with cell's style if existed
            if (is_array($cellValue) && (isset($cellValue["value"]) || isset($cellValue["style"])))
            {
                $tmpStyle = isset($cellValue["style"])?$cellValue["style"]:$style;
                $tmpValue = isset($cellValue["value"])?$cellValue["value"]:"";
            }
            else //normal thing
            {
                $tmpStyle = $style;
            ...
            $rowXML .= $this->getCellXML($rowIndex, $cellNumber, $tmpValue, $tmpStyle->getId());
            $cellNumber++;
        }
    ...
    private function getCellXML($rowIndex, $cellNumber, $cellValue, $styleId)
    {
        ...
        if (CellHelper::isNonEmptyString($cellValue)) {
            ...
            throw new InvalidArgumentException('namespace Box\Spout\Writer\XLSX\Internal -> getCellXML : Trying to add a value with an unsupported type: ' . gettype($cellValue));
        }
