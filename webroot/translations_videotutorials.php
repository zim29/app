...
$files_to_translate = array(
    array('/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson7_esp.txt' => '/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson7_eng.txt'),
    array('/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson8_esp.txt' => '/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson8_eng.txt'),
    array('/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson6_esp.txt' => '/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson6_eng.txt'),
    array('/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson9_esp.txt' => '/var/www/vhosts/devmanextensions.com/video_tutorials_texts/lesson9_eng.txt'),
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    file_put_contents($file_translated, $final_string);
    $translated_saved = true;
}

if(empty($file_to_translate))
    die("File not found");

$languages = explode("\n",file_get_contents($file_to_translate));

if(!is_file($file_translated)) {
    fopen($file_translated, "w");
}

$translated_text = file_get_contents($file_translated);

$translations = !empty($translated_text) ? explode("\n",$translated_text) : array();
...
?><html>
    ...
    <body>
        <?= $translated_saved ? '<h1>Translated saved sucessfully</h1>' : ''; ?>
        <form method="post" action="translations_videotutorials.php?file=<?= $_GET['file'] ?>">
