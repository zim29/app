...
class OpencartFormGeneratorComponent extends Component {
	...
	public function generateForm($form_view, $basic_datas, $is_trial = false)
	{
		...
		if(!$this->oc_3 && !$this->oc_2 && $ie_pro && $ie_pro_new_version) {
			$this->oc_2 = true;
			$this->version = 2;
		}
		...
		if(!empty($form_view['multi_store']))
		{
			if($system != 'cs-cart') {
				$html .= '<script type="text/javascript">
		                                ' . $this->jquery_symbol . '(document).on(\'ready\', function(){';
				$html .= $this->oc_2 ? $this->jquery_symbol . '(\'div.tab-content div.store_input\').hide();' : $this->jquery_symbol . '(\'div.content form tr.store_input\').hide();';
				$html .= 'change_store(' . $this->jquery_symbol . '("select.select_stores").val());';
				$html .= '});';
				$html .= '</script>';
			}
	...
	public function field_generate_table($theads, $fields, $table_extra, $config_data = array())
	{
		...
		$html = '<table class="list table table-bordered table-hover '.(!empty($table_extra['class']) ? $table_extra['class'] : '').'"'.(!empty($table_extra['style']) ? ' style="'.$table_extra['style'].'"' : '').'>';
		if(!empty($theads))
		{
	...
	{
		$html = '';
		if($this->oc_2) $html = '<div class="tab-content">';
